...
protected function sendRate($customerId, $productId, $rate){
	if($customerId == null) return;
	try {
		$this->client->resetParameters(false);
		$this->client->setUri('http://recommender:8080/recommender-1.0.0/ratings');
		...
		$this->client->setRawBody(Json::encode($params));
	...
	//$this->messageManager->addError('code '. $response->getStatusCode() . 'productId: ' . $productId . ' customerid: '. $customerId);
 }  
